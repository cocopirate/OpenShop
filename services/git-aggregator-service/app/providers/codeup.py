"""Codeup webhook payload parser.

Codeup push event example (simplified):
{
  "ref": "refs/heads/main",
  "repository": {"name": "order-service", "git_http_url": "https://..."},
  "commits": [
    {
      "id": "abc123",
      "message": "fix: bug",
...
def parse(payload: dict) -> NormalisedPushPayload:
    ...
    repo_name = repo_info.get("name") or payload.get("project", {}).get("name", "unknown")
    repo_url = repo_info.get("git_http_url") or repo_info.get("url")
    ref = payload.get("ref", "")
    ...
    for c in payload.get("commits") or []:
        ...
        ts_raw = c.get("timestamp") or c.get("committed_at") or datetime.now(timezone.utc).isoformat()
        try:
