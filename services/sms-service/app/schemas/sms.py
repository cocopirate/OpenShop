...
class SmsSendRequest(BaseModel):
    ...
    channel: Optional[str] = Field(None, max_length=64, description="Named channel for routing (e.g. 'internal', 'biz_a'); defaults to SMS_DEFAULT_CHANNEL")
...
class SmsConfigOut(BaseModel):
    sms_default_channel: str = Field(..., description="无 X-API-Key 时使用的默认渠道名称")
    ...
    sms_channels: Dict[str, dict] = Field(default_factory=dict, description="渠道配置（密钥已脱敏）")
    sms_client_keys: Dict[str, str] = Field(default_factory=dict, description="客户端 API Key → 渠道名称映射")


class SmsConfigUpdate(BaseModel):
    sms_default_channel: Optional[str] = Field(None, max_length=64, description="切换默认渠道")
...
class SmsChannelCreate(BaseModel):
    ...
    failure_threshold: Optional[int] = Field(None, ge=1, description="熔断阈值（连续失败次数，默认 3）")
    recovery_timeout: Optional[int] = Field(None, ge=1, description="熔断恢复等待时间（秒，默认 60）")
...
class SmsChannelUpdate(BaseModel):
    ...
    secret_id: Optional[str] = Field(None, description="SecretId（腾讯云）")
    secret_key: Optional[str] = Field(None, description="SecretKey（腾讯云）")
    failure_threshold: Optional[int] = Field(None, ge=1, description="熔断阈值")
    recovery_timeout: Optional[int] = Field(None, ge=1, description="熔断恢复等待时间（秒）")
